...
k=2;
 ...
 A=xlsread('D:\orgenal\ds1\ds1_1.xlsx','Sheet1'); 
[m,n] = size(A) ;
P = 0.70 ;
idx = randperm(m)  ;
...
% here we will start our model where we will start firtst iteration 
for ite=1:2
...
x=1;y=1;
C1_k1=[]; C1_k2=[];% this virible represent the Class {1} and Cluster {1,2,..}
C2_k1=[]; C2_k2=[];% this virible represent the Class {2} and Cluster {1,2,..}
%set matrices for the clusteres 

data_org11=[];data_org12=[];
data_org21=[];data_org22=[];
...
for i=1:size(te_inp1,1)
 ...
 dis1(3,1)  =   pdist2(te_inp1(i,:),data_org2(C2_k1(pos21_c,3),:),'euclidean'); 
 dis1(4,1)  =   pdist2(te_inp1(i,:),data_org2(C2_k2(pos22_c,3),:),'euclidean');
 [min1,pos1]= min(dis1(:,1));
 if pos1<3    
   lab(i,2)=1;
 else
   lab(i,2)=0;
